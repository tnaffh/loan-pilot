...
import { Download, Loader2, Mail, RefreshCw, ShieldCheck, Upload } from 'lucide-react';
...
const BASE = (loanId: string) => `/loans/${loanId}/collateral-agreement`;

/** The pledged collateral's signed agreement — mirror of the loan agreement card. */
export const CollateralAgreementCard = ({ loanId, user, token, hasSignature }: Props) => {
  ...
  const load = async () => {
    try {
      setAgreement(await apiFetch<AgreementView>(BASE(loanId), { token }));
    } catch (error) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const latest = await apiFetch<AgreementView>(BASE(loanId), { token });
  ...
  const generate = async () => {
    setBusy('generate');
    try {
      const view = await apiFetch<AgreementView>(BASE(loanId), { method: 'POST', body: {}, token });
      setAgreement(view);
      toast.success('Collateral agreement generated');
    } catch (error) {
  ...
  const email = async () => {
    setBusy('email');
    try {
      await apiFetch(`${BASE(loanId)}/email`, { method: 'POST', body: {}, token });
  ...
  const uploadScan = async (file: File) => {
    setBusy('upload');
    try {
      await uploadDocument(`${BASE(loanId)}/upload`, { kind: 'collateral_agreement', file }, { token });
      toast.success('Signed collateral agreement uploaded');
      await load();
    } catch (error) {
    ...
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center gap-2 text-base">
          <ShieldCheck className="size-4 text-muted-foreground" /> Collateral agreement
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        {loading ? (
          ...
          <p className="text-sm text-muted-foreground">
            No collateral agreement has been generated for this loan yet.
            {!hasSignature
